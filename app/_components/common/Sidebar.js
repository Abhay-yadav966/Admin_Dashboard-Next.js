...
const Sidebar = () => {
  return (
    <div className=" h-full border border-red-600 flex flex-col gap-5" >
        {/* image and heading */}
          ...
          <div className="pt-5 flex flex-col gap-4" >
            {
              SidebarLinks?.map((data) => (
                <Link href={data?.path} key={data?.id} >
                  <div className="flex items-center gap-2 text-white text-lg font-medium px-4 py-2 rounded-lg hover:bg-[#2e374a]" >
                    {/* icon */}
                    {data?.icon}
                    <p>{data?.title}</p>
                  </div>
